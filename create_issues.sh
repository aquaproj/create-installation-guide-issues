...
while read -r repo; do
	if echo "$repo" | grep -f created_repos.txt > /dev/null; then
		echo "[INFO] Skipped creating an issue because it was already created to this repository  repo=$repo url=https://github.com/$repo" >&2
		continue
	fi
	echo "[INFO] Creating an issue  repo=$repo url=https://github.com/$repo" >&2
